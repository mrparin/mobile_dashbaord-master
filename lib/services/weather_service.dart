...
class WeatherService {
  ...
  Future<List<WeatherForecastDay>> get7DayForecast({
    ...
    final String apiKey = dotenv.get('OPENWEATHERMAP_API_KEY', fallback: '').trim();

    // Build the query fallback order (Subdistrict -> District -> Province -> Thai Name)
    final List<String> queryFallbacks = [];
    if (subdistrictEn != null && subdistrictEn.isNotEmpty &&
        districtEn != null && districtEn.isNotEmpty &&
        provinceEn != null && provinceEn.isNotEmpty) {
    ...
    if (apiKey.isNotEmpty) {
      for (final targetLocation in queryFallbacks) {
        try {
          final String url = 'https://api.openweathermap.org/data/2.5/forecast?q=${Uri.encodeQueryComponent(targetLocation)},TH&units=metric&appid=$apiKey';
          ...
          if (response.statusCode == 200) {
            ...
            if (data.containsKey('list')) {
              ...
              for (final entry in grouped.entries) {
                ...
                // Pick weather info from midday (12:00 or closest) or the first item
                Map<String, dynamic> representativeItem = dayItems.first;
                for (final item in dayItems) {
                  final String dtTxt = item['dt_txt'].toString();
                  if (dtTxt.contains('12:00:00') || dtTxt.contains('15:00:00')) {
                  ...
                  if (pop > maxRainChance) maxRainChance = pop;
                }
                
                final double avgHumidity = totalHumidity / dayItems.length;
                final double avgWindSpeed = (totalWindSpeed / dayItems.length) * 3.6; // m/s to km/h
                
                final weatherArray = representativeItem['weather'] as List<dynamic>;
                final weatherMain = weatherArray.isNotEmpty ? weatherArray[0]['main'].toString() : 'Clouds';
              ...
              if (forecastDays.isNotEmpty) {
                print('Successfully fetched OpenWeatherMap forecast for: $targetLocation');
                return forecastDays;
              }
            }
          } else {
            print('OpenWeatherMap API returned ${response.statusCode} for "$targetLocation". Trying next fallback...');
          }
        } catch (e) {
          print('Error fetching OpenWeatherMap forecast for "$targetLocation": $e. Trying next...');
        }
      }
    } else {
  ...
  Future<List<WeatherForecastDay>?> _fetchOpenMeteoForecast(List<String> locationFallbacks) async {
    for (final locationName in locationFallbacks) {
      try {
        // 1. Geocode location name to latitude and longitude
        final geocodingUrl = 'https://geocoding-api.open-meteo.com/v1/search?name=${Uri.encodeQueryComponent(locationName)}&count=1&language=en';
        print('Trying Open-Meteo geocoding search for: $locationName');
        final geoResponse = await http.get(Uri.parse(geocodingUrl)).timeout(const Duration(seconds: 4));
        
        if (geoResponse.statusCode == 200) {
          final geoData = jsonDecode(geoResponse.body) as Map<String, dynamic>;
          if (geoData.containsKey('results') && (geoData['results'] as List).isNotEmpty) {
            final firstResult = geoData['results'][0];
            final double latitude = (firstResult['latitude'] as num).toDouble();
            final double longitude = (firstResult['longitude'] as num).toDouble();
            print('Geocoded "$locationName" to: ($latitude, $longitude)');
            
            // 2. Query forecast with resolved coordinates
            final forecastUrl = 'https://api.open-meteo.com/v1/forecast?latitude=$latitude&longitude=$longitude&daily=temperature_2m_max,temperature_2m_min,relative_humidity_2m_max,wind_speed_10m_max,precipitation_probability_max,weather_code&timezone=Asia%2FBangkok';
            ...
            if (forecastResponse.statusCode == 200) {
              ...
              if (data.containsKey('daily')) {
                ...
                final humidities = daily['relative_humidity_2m_max'] as List<dynamic>;
                final windSpeeds = daily['wind_speed_10m_max'] as List<dynamic>;
                final rainChances = daily['precipitation_probability_max'] as List<dynamic>;
                final codes = daily['weather_code'] as List<dynamic>;
                
                final List<WeatherForecastDay> list = [];
                for (int i = 0; i < times.length; i++) {
                  final int code = (codes[i] as num).toInt();
                  list.add(WeatherForecastDay(
        ...
        print('Open-Meteo geocoding/forecast error for "$locationName": $e. Trying next fallback...');
      }
    }
    return null;
  }
  ...
  String _mapWmoCodeToDescription(int code) {
    ...
    if (code >= 95 && code <= 99) return 'มีฝนฟ้าคะนองกระจายและมีฝนตกหนักบางแห่ง';
    return 'มีเมฆบางส่วน';
  }
  ...
  List<WeatherForecastDay> _generateSimulatedForecast(
    ...
    final String seedString = '${subdistrict ?? ''}-${district ?? ''}-$province';
    final int seed = seedString.codeUnits.fold(0, (prev, element) => prev + element);
    final math.Random random = math.Random(seed);

    final List<WeatherForecastDay> forecast = [];
    final List<String> conditions = ['Sunny', 'Cloudy', 'Rainy', 'Thunderstorm', 'Windy'];
    ...
    for (int i = 0; i < 7; i++) {
      ...
      final double dayHumi = (baseHumi + (random.nextDouble() - 0.5) * 6.0).clamp(40.0, 100.0);
      final double windSpeed = 5.0 + random.nextDouble() * 15.0;

      // Higher humidity -> higher rain chance
      int rainChance = ((dayHumi - 50.0) * 1.8 + random.nextInt(20)).clamp(0, 100).toInt();
